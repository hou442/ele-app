<template>
    <div class="remark">
        <Header title="订单备注" :isLeft="true"/>
        <div class="view-body">
            <section>
                <textarea maxlength="100" 
                placeholder="填写额外对餐厅和骑手小哥备注的信息"
                v-model="text"></textarea>
                <div class="switch">
                    <span class="switch-item item-line" 
                    v-for="(item,index) in radioItem" :key="index"
                ...
                <div class="switch">
                    ...
                    v-for="(item,index) in switchItem" :key="index"
                    :class="{'selected':item.select}"
...
export default {
    ...
    methods:{
        radioclick(index){
            this.radioItem.forEach(i => {
                i.select = false;
            })
            this.radioItem[index].select = !this.radioItem[index].select
        },
        switchclick(index){
            this.switchItem[index].select = !this.switchItem[index].select
        },
        submitClick(){
            let selectItems = "";
            this.radioItem.forEach(i => {
